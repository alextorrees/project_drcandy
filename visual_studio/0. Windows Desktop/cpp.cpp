cpp ..\..\src\private\sdl_wrapper\sprites.cpp
#include <cstdlib>
#include <cassert>
#include <cstdio>
...
Sprite::~Sprite()
{
    if (animacion == NULL)
    {
        // Safe checks: frame_actual may be NULL
        if (frame_actual)
        {
            ...
            delete frame_actual;
            frame_actual = nullptr;
        }
    } // Si el sprite tiene animación no hacemos nada, las animaciones se tienen que liberar a mano

    // Do NOT call IMG_Quit() here: it must be called once when the video subsystem is torn down,
    // not per-sprite.
}
...
Frame* Sprite::CreateFrame(const char* ruta)
{
    ...
    frame->tamx = frame->tamy = 0;
    frame->hx = frame->hy = 0;
    frame->duracion = 0;

    // Diagnóstico: comprobar que el fichero existe y leer los primeros 8 bytes
    FILE* f = fopen(ruta, "rb");
    if (!f)
    {
        fprintf(stderr, "[Sprite::CreateFrame] fopen failed for '%s'\n", ruta);
    }
    else
    {
        unsigned char header[8] = {0};
        size_t n = fread(header, 1, sizeof(header), f);
        fprintf(stderr, "[Sprite::CreateFrame] fopen OK, fread returned %zu bytes for '%s', header: %02X %02X %02X %02X %02X %02X %02X %02X\n",
                n, ruta,
    ...
    {
        fprintf(stderr, "[Sprite::CreateFrame] IMG_Load failed for '%s': %s\n", ruta, IMG_GetError());
        delete frame;
    ...
    {
        // Clean up surface and throw with SDL error
        SDL_FreeSurface(frame->surface);
        delete frame;
        throw custom_error(std::string("[Sprite::CreateFrame] SDL_CreateTextureFromSurface failed: ") + SDL_GetError(),
                           __FILE__, __LINE__);
    }

    // Indicamos al hardware de que este sprite va a necesitar cálculos de alpha blending
